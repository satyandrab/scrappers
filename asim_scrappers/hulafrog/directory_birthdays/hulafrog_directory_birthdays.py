...
def extract_details(url):
    ...
    birthday_party_pkg_link = "".join(re.findall(r'<a href="(.*?)" target="_blank"><strong><small>BIRTHDAY PARTY PACKAGES', html_source)).strip()
    print birthday_party_pkg_link
    ...
    data_list = [url, title, address, classes, camps, hours, places, birthdays, phone, website, mail_id, facebook, description, love_rating, image_url]
    return data_list
...
if __name__ == '__main__':
    csv_file_name = 'hulafrog_directory_birthdays.csv'
    
    data_writer = csv.writer(open(csv_file_name, 'wb'))
    data_writer.writerow(['URL', 'Title', 'Address', 'Classes', 'Camps', 'Hours', 'Places', 'Birthdays', 'Phone', 'Website','Mail ID', 'Facebook Link',
                          'Description', 'Love Rating', 'Image URL'])
    
    url = 'http://hulafrog.com/locations'
    location_link = extract_city_codes(url)
    for location_url in location_link:
        category_location_url = location_url+'/category-browse/?cat_id=1'
