...
def extract_details(url):
    ...
    other = "".join(re.findall(r'Other:(.*?)<br>', html_source)).strip()
    #print childcare
    ...
    data_list = [url, title, address, classes, other, places, birthdays, hours, phone, website, mail_id, facebook, description, love_rating, image_url]
    return data_list
...
if __name__ == '__main__':
    csv_file_name = 'hulafrog_directory_other_things.csv'
    
    data_writer = csv.writer(open(csv_file_name, 'wb'))
    data_writer.writerow(['URL', 'Title', 'Address', 'Classes', 'Other', 'Places', 'Birthdays', 'Hours', 'Phone', 'Website','Mail ID', 'Facebook Link', 'Description', 'Love Rating', 'Image URL'])
    
    url = 'http://hulafrog.com/locations'
    location_link = extract_city_codes(url)
    for location_url in location_link:
        category_location_url = location_url+'/category-browse/?cat_id=20'
