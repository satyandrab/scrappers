...
class smu:
    ...
    def load_page(self, browser):
        browser.find_element_by_xpath("//div[@class='_ph_z2 scrollContainer']//div[@class='_ph_K2'][1]//span[@class='_pe_i _pe_C']/span").click()
        browser.find_element_by_xpath("//div[@class='_ph_z2 scrollContainer']//div[@class='_ph_K2'][1]//span[@class='_pe_i _pe_C']/span").send_keys(Keys.END)
        time.sleep(10)
    ...
    def extract_data(self, browser):
        data_writer = csv.writer(open("smu_updated.csv", "wb"))
        data_writer.writerow(['First Name', 'Last Name', 'Email Address', 'Job Title'])
        dir_item = browser.find_element_by_xpath("//div[@class='_ph_z2 scrollContainer']//div[@class='_ph_K2'][last()]//span[@class='_pe_i _pe_C']")
        while dir_item:
            dir_item.click()
            time.sleep(5)
            item_name = dir_item.get_attribute('textContent').strip()
            item_name_list = browser.find_elements_by_xpath("//div[contains(@aria-label,'"+str(item_name)+"')]//preceding::span[@class='_pe_R ms-fwt-sl ms-font-color-neutralPrimary _pe_p bidi _pe_q disableTextSelection']")
            ...
            for a in range(num):
                ...
                try:
                    ...
                    if ',' in name:
                        ...
                        data_list.append(first_name.replace(u'\xf1',''))
                        data_list.append(second_name.replace(u'\xf1',''))
                    else:
                ...
                try:
                    browser.find_element_by_xpath("//span[@class='_pe_i _pe_C'][span[contains(text(),'"+str(name).strip()+"')]][1]/preceding::span[@class='_pe_i _pe_C'][1]/span").click()
                    time.sleep(5)
                except:
                    pass
            ...
            time.sleep(5)
            try:
                dir_item = browser.find_element_by_xpath("//div[@class='_ph_K2'][not(preceding-sibling::div)]//span[@class='_pe_R ms-fwt-sl ms-font-color-neutralPrimary _pe_p bidi _pe_q disableTextSelection']")
            except:
                pass
